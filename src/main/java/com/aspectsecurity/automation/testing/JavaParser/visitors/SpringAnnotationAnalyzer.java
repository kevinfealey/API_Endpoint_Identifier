...
public class SpringAnnotationAnalyzer extends VoidVisitorAdapter<Optional<PackageDeclaration>> {
	...
	private ArrayList<Parameter> handleMethodParameters(NodeList<com.github.javaparser.ast.body.Parameter> params){
		...
		for (com.github.javaparser.ast.body.Parameter param : params) {
			...
			for (AnnotationExpr annot : annots) {
				logger.debug("ANNOTATION ON PARAM: " + annot.toString()); // this returns something like this: @RequestParam("id10")
	...
	private Endpoint handleRequestMappingFound(AnnotationExpr annotation, String clazzPackageName, String methodClazzName) {
			...
			for (AnnotationAttribute attribute : annotationAttributes) {
				...
				if (attribute.getName().equals("value") || attribute.getName().equals("path")) {
					...
					for (String httpMethod : methods) {
						if (httpMethod != null) { // the array will have a bunch
													// of empty spots, unless 10
													// HTTP Methods are
													// supported
					...
					for (Parameter httpHeader : headers) {
						if (httpHeader != null) { // the array will have a bunch
													// of empty spots, unless 10
													// HTTP Methods are
													// supported
					...
					for (String outputFormat : produces) {
						if (outputFormat != null) { // the array will have a
													// bunch of empty spots,
													// unless 10 output formats
													// are supported
					...
					for (String ingestFormat : consumes) {
						if (ingestFormat != null) { // the array will have a
													// bunch of empty spots,
													// unless 10 ingest formats
													// are supported
		...
		// Add all our endpoints and associated data to an array in
		// SpringAPIIdentifier because idk
		// how to return it... we should look for a better option that storing
		// in this static object
//		SpringAPIIdentifier.addEndpoint(newEndpoint);
		logger.debug("");
		logger.debug("-----------------------------------------------");
		logger.debug("");
